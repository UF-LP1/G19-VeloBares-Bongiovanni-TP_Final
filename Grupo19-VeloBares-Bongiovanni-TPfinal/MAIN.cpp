...
const vector<string> apellidoPac = { //lo mismo pero con direcciones
...
const vector<time_t> fechanacimientoPac = { //por qu eno toma?
...
const vector<unsigned int> telefonoPac = { //por qu eno toma?
...
const vector<unsigned int> codigoPac = { //por qu eno toma?
...
stack<cPaciente*> generador_pacientes(unsigned int cantidad) { //me genero una "pila" de pacientes  con todos sus atributos random
    ...
    for (unsigned int i = 0; i < cantidad; i++) //faltaria agregar alergia y problema por eso no toma
        MisPacientes.push(new cPaciente(nomPac[randint(0, nomPac.size())], apellidoPac[randint(0, apellidoPac.size())], randint(1000000000, 9999999999), to_string(randint(1000000000, 9999999999)), (bool)randint(0, 2)));

    return MisPacientes; //me devuelve la lista de clientes que se crea de manera random
}
